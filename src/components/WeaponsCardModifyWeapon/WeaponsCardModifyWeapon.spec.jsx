...
import { mountAppWithStore, storeWithCreateCharacterView, getStoreWithGun, testM1911A1WithMods, testM1911A1, testM16, getStore } from '../../helpers/testHelpers';

import ConnectedWeaponsCardModifyWeapon from '.';

describe('Customising Firearm Intergration Tests', () => {
  // const store = getStoreWithGun(testM1911A1());
  const store = getStoreWithGun(testM16());
  let wrapper;

  beforeEach(() => {
    wrapper = mount(
      <Provider store={store}>
        <ConnectedWeaponsCardModifyWeapon
          gunObj={testM16()}
        />
      </Provider>,
    );
  ...
  // mptodo - sort out where to put the intergration tests
  it('should be possible to add custom magazine', () => {
    // console.log(wrapper.debug());
    // console.log(wrapper.find('button[children="add magazine"]').debug());
    wrapper.find('button[children="add magazine"]').simulate('click');
    // console.log(wrapper.debug());
    // console.log(wrapper.find('.customMagazineForm').debug());
    ...
    // console.log(wrapper.find('button[children="Submit"]').debug());
    wrapper.find('button[children="Submit"]').simulate('click');

    // console.log(wrapper.debug());
    // expect(wrapper.text()).toContain('test18.65');
  });

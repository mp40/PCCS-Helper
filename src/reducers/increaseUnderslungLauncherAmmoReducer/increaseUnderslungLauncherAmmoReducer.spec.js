...
import { increaseUnderslungLauncherAmmoReducer } from './index';
...
describe('increaseUnderslungLauncherAmmoReducer function', () => {
  ...
  it('should increase quantity of the magazine by one', () => {
    ...
    state = increaseUnderslungLauncherAmmoReducer(state, action);

    expect(state).toMatchObject(updatedState);
    expect(state.currentCharacter.firearms[1].launcher).toMatchObject(m203WithAmmo);
  });
});
